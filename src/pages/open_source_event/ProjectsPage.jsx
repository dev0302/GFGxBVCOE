      ...
      return (
        ...
        (!selectedStacks.length ||
          project.stacks.some((stack) =>
            selectedStacks.some(
              (sel) => stack.toLowerCase() === sel.toLowerCase(),
            ),
          )) &&
        (!selectedDifficulty ||
          ...
          <div className="mt-5 border-t border-white/[0.07] pt-5">
            <div className="flex flex-wrap items-center gap-3">
              ...
              {selectedDifficulty && (
                ...
                >
                  {difficultyOptions.find((d) => d.value === selectedDifficulty)
          ...
          {!loading &&
            ...
            filteredProjects.map((project) => (
              ...
              >
                ...
                <div className="flex items-start gap-4 pr-24">
                  <div className="min-w-0">
                    ...
                    <h2
                      onClick={() =>
                        navigate(
                          `/open-source/project/${encodeURIComponent(project._id || getProjectKey(project))}`,
                          { state: { project } },
                        )
                      }
                      className="mt-2 cursor-pointer text-xl font-semibold tracking-tight text-white transition hover:text-emerald-400"
                    >
                      {project.name}
                    </h2>
                ...
                <div className="mb-2 mt-5 flex min-h-[34px] flex-wrap items-center gap-2">
                  ...
                  {(project.stacks || []).length > 3 && (
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-md border border-emerald-400/40 bg-emerald-400/15 px-2.5 py-1.5 text-[11px] font-semibold text-emerald-300 transition hover:border-emerald-300 hover:bg-emerald-400/25 hover:text-white"
                      title="Show more stacks and view full project details"
                    >
                      +{project.stacks.length - 3} Show More
                    </button>
                  )}
                </div>
                    ...
                    <div className="mt-2 flex items-center gap-2">
                      {project.maintainerEmail && (
                        <button
                          type="button"
                          onClick={() =>
                            copyMaintainerEmail(project.maintainerEmail)
                          }
