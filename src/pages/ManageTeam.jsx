...
import { Users, Plus, Download, FileText, Edit3, Trash2, ArrowLeft, Link2, X, Printer, Mail, List } from "react-feather";
import { driveLinkToImageUrl, avatarPlaceholder, photoPreviewUrl } from "../utils/teamMemberUtils";
import { downloadTeamListPDF, downloadTeamListExcel } from "../utils/teamListExport";
...
export default function ManageTeam({ department: propDepartment, isSociety, onBack }) {
  ...
  const [selectedExportFields, setSelectedExportFields] = useState([...EXPORT_COLS]);
      ...
      for (const dept of departments) {
        ...
        for (const row of rosterData) {
          ...
          if (row.registered && row.user) {
            byEmail.set(email, { type: "registered", data: row.user, department: dept });
          } else {
  ...
  const handleGenerateInviteLink = async () => {
    ...
    try {
      ...
      if (res.success && res.data) {
        ...
        try {
          localStorage.setItem(INVITE_LINK_STORAGE_PREFIX + displayDepartment, JSON.stringify(res.data));
        } catch (_) {}
  ...
  const handleSuspendInviteLink = () => {
    ...
    suspendTeamInviteLink(inviteLinkData.token)
      .then(() => {
        ...
        try {
          localStorage.removeItem(INVITE_LINK_STORAGE_PREFIX + displayDepartment);
        } catch (_) {}
  ...
  const openEdit = (m) => {
    setEditMember(m);
    setForm(COLS.reduce((acc, k) => ({
      ...acc,
      [k]: k === "photo" ? (m.photo ?? m.image_drive_link ?? "") : (m[k] ?? ""),
    }), {}));
  };
  ...
  const handleEditSubmit = async (e) => {
    ...
    if (!form.email?.trim()) { toast.error("Email is required"); return; }
    if (!form.contact?.trim()) { toast.error("Contact is required"); return; }
    if (!form.photo?.trim()) { toast.error("Photo is required"); return; }
    setSaving(true);
    try {
      const payload = department ? { ...form, department } : form;
  ...
  return (
    <div className="min-h-screen darkthemebg pt-24 pb-16">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-bold text-white flex items-center gap-2">
                ...
                {isSociety ? `Manage society › ${displayDepartment}` : "Manage your team"}
              </h1>
              <p className="text-gray-400 text-sm mt-1">
                Department: <span className="text-cyan-300 font-medium">{displayDepartment}</span>
              </p>
            </div>
        ...
        <div className="rounded-2xl border border-gray-500/30 bg-[#1e1e2f]/80 overflow-hidden">
          {loading ? (
            ...
            const tableRows = [
              ...
              ...extraMembers.map((m) => ({ type: "teamMember", teamMember: m })),
            ];
            if (tableRows.length === 0) {
              return (
                <div className="p-12 text-center text-gray-400">
                  No one yet. Add allowed emails from Dashboard (signup config) or add team members below.
                </div>
              );
            }
            return (
                  ...
                  <tbody>
                    {tableRows.map((row) => {
                          ...
                          >
                            {COLS.map((k) => (
                              <td
                                ...
                                title={k === "photo" ? (m.photo || m.image_drive_link) : undefined}
                              >
                                {k === "name" ? (
                      ...
                      const photoUrl = row.registered
                        ? u?.image
                        : (pre?.image
                          ? (pre.image.startsWith("http") ? pre.image : `https://www.gfg-bvcoe.com${pre.image.startsWith("/") ? "" : "/"}${pre.image}`)
                          : null);
                      const cell = (k) => {
                        if (k === "name") return name;
                        if (!row.registered) return k === "email" ? row.email : "—";
                        if (k === "year") return profile?.year || profile?.yearOfStudy || "—";
                        ...
                        if (k === "non_tech_society") return profile?.non_tech_society ?? "—";
                        return "—";
                      };
                      return (
                        <tr
                        ...
                        >
                          {COLS.map((k) => (
                            <td
                              ...
                              title={k === "photo" ? (photoUrl || "") : undefined}
                            >
                              {k === "name" ? (
                                <div className="flex flex-col gap-1">
                                  <span className="truncate">{cell(k) || "—"}</span>
                                ...
                                <img
                                  src={photoUrl ? (photoUrl.startsWith("http") ? photoUrl : photoPreviewUrl(photoUrl)) : avatarPlaceholder(name)}
                                  alt={name}
                                  className="h-10 w-10 rounded-full object-cover border border-gray-500/50 shrink-0"
                                  onError={(e) => {
                                    e.target.onerror = null;
                                ...
                                <span className="truncate block">{cell(k) || "—"}</span>
                              )}
                            </td>
                          ))}
          ...
          >
            ...
            <div className="p-4 space-y-4">
              <p className="text-sm text-gray-400">
                Select columns to include in the export. Then generate PDF or Excel.
              </p>
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-gray-500/30">
                <button
                  ...
                  disabled={members.length === 0 || selectedExportFields.length === 0}
      ...
      {inviteLinkOpen && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-lg font-bold text-white mb-2">Add team by invite link</h2>
            <p className="text-sm text-gray-400 mb-4">
              Generate a link for <span className="text-cyan-300 font-medium">{displayDepartment}</span>. Anyone who opens it can fill the same form and get added to this department.
            </p>
            <button
              type="button"
            ...
            {inviteLinkData?.token && (
              <div className="mt-4 p-4 rounded-xl bg-[#252536] border border-gray-500/20">
                <p className="text-xs text-gray-400 mb-2">Share this link (valid for 12 hours):</p>
                <p className="text-sm font-mono break-all text-cyan-300 mb-2">
                  {typeof window !== "undefined" ? `${window.location.origin}/join-team/${inviteLinkData.token}` : ""}
                </p>
                <p className="text-xs text-gray-500 mb-3">
                  Expires: {inviteLinkData.expiresAt ? new Date(inviteLinkData.expiresAt).toLocaleString() : ""}
                </p>
      ...
      {editMember && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
          onClick={() => { setEditMember(null); setForm(COLS.reduce((acc, k) => ({ ...acc, [k]: "" }), {})); }}
          role="dialog"
          aria-modal="true"
        >
          <div
      ...
      {showAllTeamOpen && (
        <div
          ...
          onClick={() => { setShowAllTeamOpen(false); setSelectedDetailItem(null); }}
          role="dialog"
          aria-modal="true"
