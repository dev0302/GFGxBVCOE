...
export default function ManageEvents() {
  ...
  const handleCancelScheduledDelete = async (ev) => {
    ...
    toast
      .promise(cancelScheduledDelete(ev._id), {
  ...
  return (
    <div className="flex min-h-full w-full justify-center bg-[#1e1e2f] pb-20 px-3 sm:px-6 md:px-8 lg:px-10">
      <div className="w-full max-w-3xl py-6 sm:py-8 md:py-10 flex flex-col gap-6 sm:gap-8 md:gap-10">
        <div>
          <h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold tracking-tight text-richblack-25">
            Manage uploaded events
          </h1>
          <p className="mt-1.5 sm:mt-2 text-gray-400 text-xs sm:text-sm">
            Only events uploaded through this dashboard. Hardcoded events on the
            Events page are not listed here.
          </p>
        </div>
          ...
          {loading ? (
            ...
            <ul className="space-y-2 sm:space-y-3">
              {managedEvents.map((ev) => (
                <li
                  key={ev._id}
                  className="relative flex flex-col sm:flex-row sm:flex-wrap sm:items-center sm:justify-between gap-2 sm:gap-3 px-3 py-3 pr-24 sm:px-5 sm:py-4 sm:pr-28 rounded-lg sm:rounded-xl bg-[#252536] border border-gray-500/20 hover:border-gray-500/40 transition-colors"
                >
                  <div className="min-w-0 flex-1 order-1">
                    <span className="font-medium text-richblack-25 block truncate text-sm sm:text-base">
                      {ev.title}
                    </span>
          ...
          >
            ...
            >
              <div className="flex items-center gap-2 sm:gap-3 mb-3 sm:mb-4">
                ...
                <h3 className="text-base sm:text-lg font-semibold text-richblack-25">
                  Delete event?
                </h3>
              </div>
              <p className="text-gray-400 text-xs sm:text-sm mb-4 sm:mb-6">
                This event will stay in the database for 10 days. After 10 days
                it will be automatically deleted if not cancelled. You can
                cancel the deletion anytime before then.
              </p>
